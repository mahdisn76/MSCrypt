#include <iostream>
#include <string>
#include <vector>
#include <sstream>
#include <iomanip>
#include "utilities.h"
#include "RoundKeyGenerator.h"

using namespace std;


uint8_t sbox(uint8_t in)
{
	constexpr uint8_t sbox[256] = {
		 };
	
	return sbox[in];
}

uint32_t pbox(uint32_t in)
{
	constexpr size_t perm[32] = { 4, 28, 15, 3, 6, 29, 23, 9, 11, 30, 14, 31, 12, 10, 5, 24, 21, 13, 18, 2, 7, 26, 25, 22, 19, 16, 8, 0, 17, 1, 27, 20};

	uint32_t out = 0;
	for (auto i = 0; i < 32; i++)
	{
		out |= ((in >> i) & 0b1) << perm[i];
	}

	return out;
}

uint32_t F(uint32_t in, uint32_t roundKey)
{
	in ^= roundKey;
	
	vector<uint8_t> parts = divide(in);

	for (auto i = 0; i < 4; i++)
		parts[i] = sbox(parts[i]);

	return pbox(merge(parts));
}

string enc(string plain, string key)
{
	RoundKeyGenerator rkGen(key);

	uint32_t p1, p2, p3;
	divide(plain, p1, p2, p3);

	for (auto i = 0; i < 12; i++)
	{
		uint32_t roundKey = rkGen.getEncRoundKey(i);
		auto previousP1 = p1;
		
		p1 = F(p3, roundKey) ^ p2;
		p2 = p3;
		p3 = previousP1;
	}

	return merge(p1, p2, p3);
}

string dec(string plain, string key)
{
	RoundKeyGenerator rkGen(key);

	uint32_t p1, p2, p3;
	divide(plain, p1, p2, p3);

	for (auto i = 0; i < 12; i++)
	{
		uint32_t roundKey = rkGen.getDecRoundKey(i);
		
		auto previousP1 = p1;
		auto previousP3 = p3;

		p1 = p3;
		p3 = p2;
		p2 = previousP1 ^ F(p3, roundKey);
	}

	return merge(p1, p2, p3);
}


int main()
{
	string key = "1010101010111011111010010001100000100111001101101100110011000101";
	string plainText = "123456FF1234123456123456"; //24 Hex char

	cout << "plain:  " << plainText << endl;

	auto cipher1 = enc(plainText, key);

	cout << "cipher: " << cipher1 << endl;

	
	plainText = "133456FF1234123456123456"; //24 Hex char

	cout << "plain:  " << plainText << endl;

	auto cipher2 = enc(plainText, key);

	cout << "cipher: " << cipher2 << endl;

	cout << countDifferentBits(cipher1, cipher2) << "bits" << endl;

	return 0;
}	