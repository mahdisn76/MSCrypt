#include <vector>
#include "MSCrypt.h"
#include "utilities.h"
using namespace std;

MSCrypt::MSCrypt(uint64_t key, std::string IV, Mode mode):
	key_(key), IV_(IV), mode_(mode)
{
	rkGen_ = make_unique<RoundKeyGenerator>(key_);
}

uint8_t MSCrypt::sbox(uint8_t in)
{
	constexpr uint8_t sbox[256] = {
		 };

	return sbox[in];
}

uint32_t MSCrypt::pbox(vector<uint8_t> in)
{
	auto y0 = in[1] ^ in[2] ^ in[3];
	auto y1 = in[0] ^ in[2] ^ in[3];
	auto y2 = in[0] ^ in[1] ^ in[3];
	auto y3 = in[1] ^ in[2] ^ in[0];

	return (y0 << 24) ^ (y1 << 16) ^ (y2 << 8) ^ y3;
}

uint32_t MSCrypt::F(uint32_t in, uint32_t roundKey)
{
	in ^= roundKey;

	vector<uint8_t> parts = divide(in);

	for (auto i = 0; i < 4; i++)
		parts[i] = sbox(parts[i]);

	return pbox(parts);
}


string MSCrypt::enc_block(string plain)
{
	uint32_t p1, p2, p3;
	divide(plain, p1, p2, p3);

	for (auto i = 0; i < 12; i++)
	{
		uint32_t roundKey = rkGen_->getEncRoundKey(i);
		auto previousP1 = p1;

		p1 = F(p3, roundKey) ^ p2;
		p2 = p3;
		p3 = previousP1;
	}

	return merge(p1, p2, p3);
}

string MSCrypt::dec_block(string cipher)
{
	uint32_t p1, p2, p3;
	divide(cipher, p1, p2, p3);

	for (auto i = 0; i < 12; i++)
	{
		uint32_t roundKey = rkGen_->getDecRoundKey(i);

		auto previousP1 = p1;
		auto previousP3 = p3;

		p1 = p3;
		p3 = p2;
		p2 = previousP1 ^ F(p3, roundKey);
	}

	return merge(p1, p2, p3);
}

string MSCrypt::enc(string plain)
{
	if (mode_ == Mode::Block)
		return enc_block(plain);

	return ofb(plain);
}

string MSCrypt::dec(string cipher)
{
	if (mode_ == Mode::Block)
		return dec_block(cipher);

	return ofb(cipher);
}

string MSCrypt::ofb(string text)
{
	vector<string> plains = split(text, 24);
	string output_text = "", in = IV_;
	
	for (auto plain : plains)
	{
		in = enc_block(in);
		output_text += XOR(IV_, plain);
	}

	return output_text;
}